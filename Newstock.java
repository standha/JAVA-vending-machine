...
public Newstock() {
   ...
   JLabel lbl3 = new JLabel("변경 재고 입력:");
   ...
   //버튼 생성
    ...
    JButton change=new JButton("음료 재고 변경");
   ...
   p.add(change);     //변경 버튼 추가
    ...
    change.addActionListener(new ActionListener() {
           ...
           public void actionPerformed(ActionEvent e) {
              ...
              int i1=Integer.valueOf(txt2.getText());    // 메뉴 변호 
              int i=Integer.valueOf(txt3.getText());     //재고 변경
              
            if(i1 > 4) {        //메뉴 번호가 4를 넘어갔을 때
               JOptionPane.showMessageDialog(null, "다시입력", "번호를 확인하세요", JOptionPane.DEFAULT_OPTION);
               frame.dispose();      // 창 안보이게 하기
               new Admin();             //관리자 페이지로 이동
              }
             else {
            	 Drink.setproductstoct(i1,i);
                 frame.dispose();     // 창 안보이게 하기
               new Admin();             //관리자 페이지로 이동
           }
           }
